...
export default function RelationalStage({ schemes, selected, onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    import("d3").then((d3) => {
      ...
      const sim = d3.forceSimulation<SchemeNode>(nodes)
        ...
        .force("collide", d3.forceCollide<SchemeNode>(32))
      ...
      nodeEls
        ...
        .attr("dy", (d) => (d.maturity === "Pioneer" ? 24 : 20))
        .attr("text-anchor", "middle")
        .attr("font-size", (d) => (d.maturity === "Pioneer" ? "9.5px" : "8.5px"))
        .attr("font-family", "JetBrains Mono, monospace")
        .attr("fill", (d) => MATURITY_COLOR[d.maturity] || "#6B7280")
        .attr("font-weight", "500")
        .text((d) => d.label);

      // Country code sub-label
      nodeEls
        .append("text")
        .attr("dy", (d) => (d.maturity === "Pioneer" ? 34 : 29))
        .attr("text-anchor", "middle")
        ...
        .on("mouseenter", (event, d) => {
          ...
          linkEls
            .attr("stroke", (l) => {
              ...
              return l.type === "interop" ? "rgba(56,189,248,0.12)" : "rgba(31,41,55,0.2)";
            })
            .attr("stroke-width", (l) => {
              const src = (l.source as SchemeNode).id;
              const tgt = (l.target as SchemeNode).id;
  ...
  return (
    <div className="relative w-full h-full bg-rr-black overflow-hidden">
      ...
      {/* ── Overlay: graph type label ── */}
      <div className="absolute top-4 left-4 z-10 glass-card px-3 py-2 flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-rr-cyan animate-pulse" />
        <span className="text-[11px] font-mono text-rr-cyan">RELATIONAL</span>
        <span className="text-[11px] text-rr-muted ml-1">Payment Flow Network</span>
      </div>

      {/* ── Overlay: scheme count ── */}
      <div className="absolute top-4 left-52 z-10 glass-card px-3 py-2">
        <span className="text-[11px] font-mono text-rr-cyan">{schemes.length}</span>
        <span className="text-[11px] text-rr-muted ml-1">nodes visible</span>
      </div>

      {/* ── Legend ── */}
      <div className="absolute bottom-20 left-4 z-10 glass-card px-3 py-2.5 space-y-1.5">
        ...
        <div className="border-t border-rr-border pt-2 mt-2 space-y-1.5">
          <div className="text-[10px] text-rr-muted font-mono uppercase tracking-widest mb-1">
            Edges
          </div>
          <div className="flex items-center gap-2">
            <span className="h-px w-5 flex-none" style={{ background: "rgba(56,189,248,0.7)" }} />
            <span className="text-[10px] text-rr-dim">Interop link</span>
          </div>
          <div className="flex items-center gap-2">
            <span
              className="h-px w-5 flex-none"
              style={{ background: "rgba(31,41,55,0.9)", borderTop: "1px dashed rgba(100,116,139,0.5)" }}
            />
            <span className="text-[10px] text-rr-dim">Regional cluster</span>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="absolute bottom-20 right-4 z-10 glass-card px-3 py-2 text-[11px] text-rr-muted hover:text-rr-cyan transition-colors font-mono"
      >
        Reset View
      </button>

      {/* ── Interaction hint ── */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 text-[10px] text-rr-muted/50 font-mono pointer-events-none">
